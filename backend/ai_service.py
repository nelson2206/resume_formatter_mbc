...
def extrae_perfil_cv(
    ...
    if idioma == "en":
        ...
        instruccion_idioma = (
            ...
            "Usa ortografía correcta del español, con TODAS las tildes y signos (á, é, í, ó, ú, ñ), AUNQUE el CV venga "
            "sin tildes: debes corregirlas. "
...
6. "resumen_profesional": Genera un resumen ejecutivo en UNA sola línea/párrafo continuo, SIN saltos de línea (no uses '\n' ni '\\n' dentro del texto), con un MÁXIMO ESTRICTO de 45 palabras. APUNTA a 30-45 palabras (aprovecha el espacio sintetizando info real del CV; NO entregues un resumen telegráfico de pocas palabras). VALIDACIÓN: antes de responder, cuenta las palabras del resumen; si supera 45, condénsalo hasta cumplir. Nunca entregues más de 45 palabras.
...
8. "experiencia_profesional": Lista TODAS las responsabilidades, funciones y logros que aparezcan en el CV. Redacta cada bullet de forma CONCISA: UNA sola idea o actividad por bullet, en 1 frase breve (orientativamente 12 a 22 palabras) con wording ejecutivo. EVITA párrafos largos y bullets que encadenen varias acciones. NO antepongas guiones, viñetas ni "-" al inicio de cada bullet: entrega SOLO el texto (sin "- "). ATENCIÓN: esta ampliación es SOLO de redacción; está terminantemente PROHIBIDO añadir herramientas, cifras, porcentajes, clientes, sectores, tecnologías, resultados o responsabilidades que NO estén en el CV. No crees actividades nuevas para rellenar espacio. NO hay número mínimo de bullets: si el CV es escueto, devuelve solo los que correspondan (es preferible pocos bullets bien redactados a inventar) y añade una alerta indicando que la experiencia documentada es limitada.
   ...
   - LÍMITE DE EXTENSIÓN (ESTRICTO E INVIOLABLE): la suma total de palabras de TODOS los bullets de "experiencia_profesional" NO debe superar las 260 palabras. VALIDACIÓN OBLIGATORIA: antes de entregar el JSON, CUENTA el total de palabras sumando todos los bullets; si el total supera 260, DEBES recortar o condensar hasta quedar en 260 o menos, y recién entonces responder. Bajo ninguna circunstancia entregues más de 260 palabras. Si el contenido declarado en el CV excede ese límite: (a) si se entregó "contexto_proyecto", prioriza y conserva las experiencias y actividades MÁS relevantes para ese rol/descripción, condensando o resumiendo las menos relevantes; (b) si NO hay contexto, sintetiza y resume consolidando actividades similares. NUNCA recortes de forma que se pierda información esencial: prefiere condensar la redacción antes que eliminar logros o responsabilidades importantes. Si tuviste que resumir por exceder el límite, añade una alerta indicándolo.
   - ORDEN POR RELEVANCIA (IMPORTANTE): Ordena los bullets de "experiencia_profesional" de mayor a menor relevancia respecto al "contexto_proyecto" o la instrucción recibida: coloca SIEMPRE PRIMERO las actividades, logros y responsabilidades que MÁS se relacionen con lo solicitado, y deja al final las menos relevantes. Si no se entregó contexto, ordena de lo más estratégico/senior a lo más operativo. Es OBLIGATORIO reordenar por relevancia: NO conserves el orden en que las actividades aparecen en el CV. Este orden es clave porque, si hay que recortar por el límite de palabras, se conservan primero los bullets más relevantes.
...
- "experiencia_profesional": bullets CORTOS y concisos (1 idea por bullet, ~12-22 palabras); muchos bullets breves mejor que pocos largos. Máximo 260 palabras en total (prioriza por relevancia al contexto o sintetiza si excede).
